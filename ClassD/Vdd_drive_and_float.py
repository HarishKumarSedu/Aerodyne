...
def vdd_drive_and_float():
  ...
  meas = VMEASURE(signal="ADDR", reference="PGND", expected_value=0.6, error_spread=0.05)
  print(f"vdd_drive = {meas:.3f} ")
  VMEASURE(signal="ADDR", reference="PGND", expected_value=float('Inf'))
  meas = VMEASURE(signal="IODATA0", reference="PGND", expected_value=0.6, error_spread=0.05)
  print(f"vdd_float = {meas:.3f} ")
  VMEASURE(signal="IODATA0", reference="PGND", expected_value=float('Inf'))
