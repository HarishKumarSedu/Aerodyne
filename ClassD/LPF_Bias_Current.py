...
def lpf_bias_current():
    ...
    measured_value = AMEASURE(signal="IODATA1", reference="ADDR", expected_value=expected_value, error_spread=error_spread)
    I2C_WRITE(device_address="0x38", field_info={'fieldname': 'test_sel', 'length': 4, 'registers': [{'REG': '0x15', 'POS': 0, 'RegisterName': 'ANA_TESTMUX_SEL', 'RegisterLength': 8, 'Name': 'test_sel[3:0]', 'Mask': '0xF', 'Length': 4, 'FieldMSB': 3, 'FieldLSB': 0, 'Attribute': 'NNNNNNNN', 'Default': '0x00', 'User': '00000000', 'Clocking': 'SMB', 'Reset': 'C', 'PageName': 'PAG1'}]}, write_value=0x00)
    ...
    print(f'Expected  LPF bias current (p2n) at "IODATA1" wrt "ADDR": {expected_value /  1e-9} nA')
    print(f'Measured LPF bias current (p2n) at "IODATA1" wrt "ADDR": {measured_value /  1e-9} nA')
    AMEASURE(signal="IODATA1", reference="ADDR", expected_value=float('Inf')) # open switch
