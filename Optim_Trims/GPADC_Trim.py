...
def gpadc_vbat_gain_off_trim():
    ...
    elif delta_off > off_max_code:
    	delta_off = off_max_code
    else:
        delta_off = delta_off
    gpadc_off = round(vbat_pretrim_offset + delta_off)
    gpadc_offset_code = dec_to_2complement(gpadc_off,vbat_offset_otp_length,False)
    ...
    print(f'GPADC-TEMP PRETRIM GAIN OFFSET :~')
    ...
    I2C_WRITE(device_address="0x38",field_info={'fieldname': 'sar_temp_avg_sel', 'length': 2, 'registers': [{'REG': '0xDE', 'POS': 2, 'RegisterName': 'SAR ADC settings ', 'RegisterLength': 8, 'Name': 'sar_temp_avg_sel[1:0]', 'Mask': '0xC', 'Length': 2, 'FieldMSB': 1, 'FieldLSB': 0, 'Attribute': 'NNNNNNNN', 'Default': '0x0F', 'User': '00000000', 'Clocking': 'SMB', 'Reset': 'C', 'PageName': 'PAG0'}]},write_value=0x0)
