...
def ocp_ls_trim():
  ...
  if trigger:
    ...
    # sweep code from 0xF to 0x8, then 0x0 to 0x7
      for code in list(list(range(2**bit_width -1,2**bit_width//2 -1,-1))+list(range(0,2**bit_width//2,1))) : 
          ...
          trigger = VTRIG_LH(signal="IODATA0", reference="GND", threshold=1.8,expected_value=0.9) # digital signal trigger level is 1.8 
  ...
  I2C_WRITE(device_address="0x38", field_info={'fieldname': 'dig_test_sel', 'length': 7, 'registers': [{'REG': '0x04', 'POS': 0, 'RegisterName': 'DIGITAL_TEST_SETTINGS_2', 'RegisterLength': 8, 'Name': 'dig_test_sel[6:0]', 'Mask': '0x7F', 'Length': 7, 'FieldMSB': 6, 'FieldLSB': 0, 'Attribute': '0NNNNNNN', 'Default': '0x00', 'User': '00000000', 'Clocking': 'SMB', 'Reset': 'C', 'PageName': 'PAG1'}]}, write_value=0x0) # FSYN ana_ds_mid_up
