...
def ana_reference_th():
  ...
  test_blk = {
    ...
    "as_bg_vref_0v6_gndref" :   { "test_selection_code" : 6, "target" : 0.6, "unit" : "V"  , "error_percentage": 5e-2 },
    "as_bg_vref_0v4_gndref" :   { "test_selection_code" : 7, "target" : 0.4, "unit" : "V"  , "error_percentage": 5e-2},
    "ref_0v6"               :   { "test_selection_code" : 10, "target" : 0.6, "unit" : "V"  , "error_percentage": 5e-2},
  ...
  for ref_test_name, ref_test in test_blk.items():
    ...
    if ref_test.get('unit','') == 'V':
      ...
      sleep(0.01)
      measured_value = VMEASURE(signal="IODATA1", reference="GND", expected_value=target_value,error_spread=target_value*error_percentage) - buffer_offset
