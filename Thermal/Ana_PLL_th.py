...
def ana_pll_th():
  ...
  for pll_test_name, pll_test in test_blk.items():
    measured_value = 0 # default 
    # if the test is voltage measure select multimeter / if there is no unit default is ''
    if pll_test.get('unit','') == 'V':
      I2C_WRITE(device_address="0x38",field_info={'fieldname': 'test_sel', 'length': 4, 'registers': [{'REG': '0x15', 'POS': 0, 'RegisterName': 'ANA_TESTMUX_SEL', 'RegisterLength': 8, 'Name': 'test_sel[3:0]', 'Mask': '0xF', 'Length': 4, 'FieldMSB': 3, 'FieldLSB': 0, 'Attribute': 'NNNNNNNN', 'Default': '0x00', 'User': '00000000', 'Clocking': 'SMB', 'Reset': 'C', 'PageName': 'PAG1'}]},write_value=pll_test.get('test_selection_code',1)) # test selection 
      ...
      sleep(0.01)
      target_value = pll_test.get('target',0) # default is 0
      error_percentage = pll_test.get('error_percentage',0) # default is 0
